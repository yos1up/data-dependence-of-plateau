...
void routine(string filename){
	...
	for(int epoch=0;epoch<1000000;epoch++){
		random_shuffle(irisTrain.begin(), irisTrain.end());
		for(int i=0;i<irisTrain.size();i++){ //オンライン学習
			mlp.update(irisTrain[i], "squared");
		}
		if (epoch % 100==0){
			...
			ZMaxOvl = mlp.calculateZMaxOverlap(irisTrain);
			HStats = mlp.calculateHStats(irisTrain);
