...
void routine(string filename){
	...
	tupleData mnist = loadMnistData(labelList);
	//TODO: random_shuffle は srand に依存するかは実は実装によるらしく、注意！ 
	//shuffleに乱数生成器を渡すようにすべきか。
	//tupleData irisTrain = get<0>(splitData(iris, 0.6));
	tupleData mnistTrain = mnist;
	
	//mnistTrain = onehottify(mnistTrain, labelList.size());
	MLP mlp_t(N, K, O);
	mnistTrain = passTeacher(mnistTrain, mlp_t);
		tuple<double, double, double, double, vd> HStats = mlp_t.calculateHStats(mnistTrain);
		vd HOSV = get<4>(HStats);
		cerr << "teacher HOSV: " << HOSV[0] << " " << HOSV[1] << " " << HOSV[2] << endl;


	cerr << "mnistTrain.size() == " << mnistTrain.size() << endl;
	/*cerr << to_string(get<0>(mnistTrain[0])) << endl;
	cerr << to_string(get<1>(mnistTrain[0])) << endl;*/
	...
	for(int epoch=0;epoch<1000;epoch++){
		random_shuffle(mnistTrain.begin(), mnistTrain.end());
		for(int i=0;i<mnistTrain.size();i++){ //オンライン学習
			mlp.update(mnistTrain[i], "squared");
		}
		if (epoch % 1==0){
			...
			ZMaxOvl = mlp.calculateZMaxOverlap(mnistTrain);
			HStats = mlp.calculateHStats(mnistTrain);
